...
import { Lead, CallLog, B2BAgent } from '../../types/rbac';
import {
  ShieldAlert,
  Users,
  Search,
...
export const SuperAdminModule: React.FC<SuperAdminModuleProps> = ({ initialView = 'dashboard' }) => {
  ...
  const [selectedAgent, setSelectedAgent] = useState<B2BAgent | null>(null);
  ...
  const openAgentView = (agent: B2BAgent) => {
            ...
            <div className="flex flex-col gap-4 rounded-[2rem] border border-slate-800 bg-slate-900/80 p-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="relative w-full sm:w-auto">
                  ...
                  <input value={agentSearch} onChange={(e) => setAgentSearch(e.target.value)} placeholder="Search agents" className="w-full rounded-2xl border border-slate-700 bg-slate-950/70 py-3 pl-10 pr-4 text-sm text-slate-200" />
                </div>
              </div>
              ...
              <div className="overflow-x-auto rounded-[1.75rem] border border-slate-800 bg-slate-950/80">
                <table className="min-w-full text-left text-sm">
                  <thead className="bg-slate-900 text-slate-400 text-xs uppercase tracking-[0.18em]">
                    <tr>
                      <th className="py-5 px-6 text-base">Agent ID</th>
                      <th className="py-5 px-6 text-base">Agency</th>
                      <th className="py-5 px-6 text-base">Owner</th>
                      <th className="py-5 px-6 text-base">Email</th>
                      <th className="py-5 px-6 text-base">Phone</th>
                      <th className="py-5 px-6 text-base">Status</th>
                      <th className="py-5 px-6 text-base">City</th>
                      <th className="py-5 px-6 text-base">Joined</th>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {filteredAgents.map((agent) => (
                      <tr key={agent.id} className="hover:bg-slate-900/80 transition-colors">
                        <td className="py-5 px-6 font-semibold text-base text-white">{agent.id}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.agencyName}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.ownerName}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.email}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.phone}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.status}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.city}</td>
                        <td className="py-5 px-6 text-base text-slate-300">{agent.joinedDate}</td>
                        <td className="py-5 px-6 text-base text-slate-300">
                          <button onClick={() => openAgentView(agent)} className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:border-sky-500">View</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {clientRows.map((client) => (
                      <tr key={client.id} className="hover:bg-slate-900/80 transition-colors">
                        ...
                        <td className="py-5 px-6 text-base text-slate-300"> <button onClick={() => openView(client as Lead)} className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-300 transition hover:border-sky-500 hover:text-white">View</button> </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
    ...
    <div className={`min-h-screen px-4 py-6 text-slate-100 sm:px-6 lg:px-8 ${isDark ? 'bg-slate-950' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {isViewOpen && selectedLead && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-4xl overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950 shadow-[0_20px_80px_rgba(0,0,0,0.65)]">
            <div className="bg-slate-900/95 px-6 py-5 backdrop-blur-xl">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs uppercase tracking-[0.25em] text-slate-500">Lead Profile</div>
                  <h3 className="mt-2 text-3xl font-bold text-white">{selectedLead.contactName}</h3>
                  <div className="mt-2 text-sm text-slate-400">{selectedLead.companyName} • Lead ID: {selectedLead.id}</div>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                  <span className="rounded-3xl bg-emerald-500/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-300">{selectedLead.status}</span>
                  <button onClick={() => setIsViewOpen(false)} className="h-11 w-11 rounded-full border border-slate-700 bg-slate-950 text-slate-300 transition hover:border-slate-600 hover:bg-slate-900">✕</button>
                </div>
              </div>
              <div className="mt-5 flex flex-wrap gap-3 text-sm text-slate-400">
                <span className="rounded-full border border-slate-800 bg-slate-900 px-4 py-2">{selectedLead.assignedTo || 'Unassigned'}</span>
                <span className="rounded-full border border-slate-800 bg-slate-900 px-4 py-2">{selectedLead.source}</span>
                <span className="rounded-full border border-slate-800 bg-slate-900 px-4 py-2">Created {selectedLead.createdAt}</span>
              </div>
            </div>

            <div className="grid gap-6 bg-slate-950 px-6 py-6 lg:grid-cols-[1.3fr_0.9fr]">
              <div className="space-y-6">
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Contact Info</div>
                      <p className="mt-1 text-slate-500">Primary contact details and communication preferences.</p>
                    </div>
                    <div className="rounded-full bg-slate-800 px-4 py-2 text-xs uppercase tracking-[0.2em] text-slate-300">Client</div>
                  </div>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Phone</div>
                      <div className="mt-2 text-base font-semibold text-white">{selectedLead.phone}</div>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Email</div>
                      <div className="mt-2 text-base font-semibold text-white">{selectedLead.email}</div>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Nationality</div>
                      <div className="mt-2 text-base font-semibold text-white">Chinese</div>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Communication</div>
                      <div className="mt-2 text-base font-semibold text-white">{selectedLead.source.toLowerCase().includes('whatsapp') ? 'WhatsApp' : 'Email'}</div>
                    </div>
                  </div>
                </div>

                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Lead Snapshot</div>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Case Manager</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedLead.assignedTo || 'Sofia Rodriguez'}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Last Activity</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedLead.lastActivity || 'No recent activity'}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Service</p>
                      <p className="mt-2 text-base font-semibold text-white">Non-Lucrative Visa (NLV)</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Status</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedLead.status}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Case Notes</div>
                  <div className="mt-4 rounded-3xl bg-slate-950 p-5 text-sm leading-6 text-slate-300">
                    {selectedLead.notes || 'No notes available. Add case comments to keep this lead moving.'}
                  </div>
                </div>
                ...
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Quick Actions</div>
                      <p className="text-xs text-slate-500">Manage status and follow-up.</p>
                    </div>
                    <button className="rounded-3xl bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400">Change Status</button>
                  </div>
                  <div className="mt-4 grid gap-3">
                    <button className="rounded-3xl border border-slate-800 bg-slate-950 px-4 py-3 text-left text-sm text-slate-200 hover:border-slate-700">Send message</button>
                    <button className="rounded-3xl border border-slate-800 bg-slate-950 px-4 py-3 text-left text-sm text-slate-200 hover:border-slate-700">Create follow-up task</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      {isAgentViewOpen && selectedAgent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-4xl overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950 shadow-[0_20px_80px_rgba(0,0,0,0.65)]">
            <div className="bg-slate-900/95 px-6 py-5 backdrop-blur-xl">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs uppercase tracking-[0.25em] text-slate-500">Agent Profile</div>
                  <h3 className="mt-2 text-3xl font-bold text-white">{selectedAgent.agencyName}</h3>
                  <div className="mt-2 text-sm text-slate-400">Agent ID: {selectedAgent.id} • Owner: {selectedAgent.ownerName}</div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="rounded-3xl bg-slate-800 px-4 py-2 text-xs uppercase tracking-[0.2em] text-slate-300">{selectedAgent.status}</span>
                  <button onClick={() => setIsAgentViewOpen(false)} className="h-11 w-11 rounded-full border border-slate-700 bg-slate-950 text-slate-300 transition hover:border-slate-600 hover:bg-slate-900">✕</button>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-6 bg-slate-950 px-6 py-6 lg:grid-cols-[1.3fr_0.9fr]">
                ...
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  ...
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-950 p-4">
                      ...
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.email}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Phone</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.phone}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">City</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.city}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Country</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.country}</p>
                    </div>
                  </div>
                </div>

                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Financial Snapshot</div>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Wallet Balance</p>
                      <p className="mt-2 text-base font-semibold text-white">${selectedAgent.walletBalance.toLocaleString()}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Credit Limit</p>
                      <p className="mt-2 text-base font-semibold text-white">${selectedAgent.creditLimit.toLocaleString()}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Commission</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.commissionRate}%</p>
                    </div>
                    <div className="rounded-3xl bg-slate-950 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Bookings</p>
                      <p className="mt-2 text-base font-semibold text-white">{selectedAgent.totalBookings}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Notes & Activity</div>
                  <div className="mt-4 rounded-3xl bg-slate-950 p-5 text-slate-300">
                    <div className="text-sm text-slate-200 font-semibold">Assigned owner</div>
                    <div className="mt-2 text-sm text-slate-400">{selectedAgent.ownerName}</div>
                  </div>
                  <div className="mt-4 rounded-3xl bg-slate-950 p-5 text-slate-300">
                    <div className="text-sm text-slate-200 font-semibold">Recent activity</div>
                    <div className="mt-2 text-sm text-slate-400">Agent has been actively managing bookings and onboarding new clients.</div>
                  </div>
                </div>
                ...
                <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/70 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Actions</div>
                      <p className="text-xs text-slate-500">Quick access controls.</p>
                    </div>
                    <button className="rounded-3xl bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400">Message Agent</button>
                  </div>
                  <div className="mt-4 grid gap-3">
                    <button className="rounded-3xl border border-slate-800 bg-slate-950 px-4 py-3 text-left text-sm text-slate-200 hover:border-slate-700">View bookings</button>
                    <button className="rounded-3xl border border-slate-800 bg-slate-950 px-4 py-3 text-left text-sm text-slate-200 hover:border-slate-700">Create contract note</button>
                  </div>
                </div>
              </div>
            </div>
